        ...
        genEventCard('search', 'さがします', 'img',
            function(card,player,modal,def){ 
                modal.open(player.deck.openAll(),{"limit":1,"needs":1,"card":card, "player":player}).result.then(
                    function(rsl){
                        rsl = rsl[0];
                        player.hand.push(player.deck.draw(rsl));
        ...
        genEventCard('umeyo', 'フィールドにある羊のコピーをフィールドに置く', 'img',
            function(card,player,modal,def){ 
                ...
                modal.open(player.field.openAll(),{"limit":1,"needs":1,"card":card, "player":player}).result.then(
                    function(rsl){
                        var crowdIdx = player.field.open(rsl[0]+1)[0]["crowd"];
                        var stat = player.field.put(player.crowd[crowdIdx].draw());
                        player.discard(player.hand.indexOf(card));
                        def.resolve('umeyo');
                        // return stat;
                    },
        ...
        genEventCard('gousei', 'ごうせい', 'img',
            function(card,player,modal,def){ 
                modal.open(player.field.openAll(),{"limit":7,"needs":0,"card":card, "player":player}).result.then(
                    function(rsl){
                         var rank =[{"name":1},{"name":3},{"name":10},{"name":30},{"name":100},{"name":300},{"name":1000}];
                         var sum = 0;
                         var target = [];
                         for (var i=0; i < rsl.length; i++){
                             target.push(player.field.open(rsl[i]+1 ,1)[0]);    
                             sum += player.field.open(rsl[i]+1 ,1)[0].rank;
                         }
                         ...
                         for (var j=0 ; j < rank.length; j++){
                             if (rank[j]["name"] <= sum){
                                 size = j;
                             console.log(rank[j]["name"]);
                             console.log(j);
                             }
                         }
                         
                         if (size >= 1){
                         modal.open(rank.slice(0,size+1),{"limit":1,"needs":1}).result.then(
                             function(result){
                                 console.log(result)
                                 player.field.put(player.crowd[result].draw())
